...
Recall that mean deviations, $d_i = x_i - \mu$ and $d_i = x_i - \bar{x}$ frame our discussion of the data in terms of the **center** of the data.

We lack information, however, on whether the mean deviation is "large" or "small" relative to the dispersion of the data.

The **z-score** is a measure of the mean deviation of an observation relative to the standard deviation of the variable.

In the population, the z-score for an observation is:

$$Z = \frac{x_i - \mu}{\sigma}$$ In the sample, the Z-score for an observation is:

$$Z = \frac{x_i - \bar{x}}{s}$$ The Z-score provides a measure in **standard deviation units**.
...
If a distribution has a mean and variance, you can estimate the Z-score.
...
The next step is estimating the z-score of each observation. We then output the first five observations to the table.
