...
Assume that the $a = 10$ and $b=30$ then the PDF of the continuous
uniform is:

$$P(x|a,b)  = \frac{1}{b-a} \quad \text{for} \, x\in[a,b]$$
$$P(x|10,30)  = \frac{1}{30-10} = 0.05 \quad \text{for} \, x\in[10,30]$$

We estimate this probability manually and using the **duinf** function
...
We estimate this probability manually and using the **punif** function.
Here we estimate
...
We also find $P(X \le 20)$ by manually and using the the cumulative
density function. Note the we are evaluating the area between 20 and the
lower bound of the continuous uniform.
...
uni <- tibble(inv_cdf <- qunif(p, min = a, max = b),
              cdf <- punif(inv_cdf, min = a, max = b))
