...
In the following code, we create a data frame with for the reactions to the administration of a drug. We calculate the relative frequency of each type of reaction by:
...
reactions <- tibble("reaction" = c("None", "Mild", "Moderate", "Severe"),
                    "cases"    = c(2420, 7502, 1149, 32))
...
#Align - left or center alignment of columsn
#Col.names - use these column names instead of data frame names
#format.args - use a comma for large numbers, no scentific noation
...
The union of the two events would be $C \cup D \in \{4, 5, 6, 8, 10 \}$.
...
Recall from the previous discussion of the union of two events that $C \in \{4, 6, 8 \}$ and $D \in \{4, 5, 8, 10 \}$. The intersection of C and D is $C \cap D \in \{4, 8 \}$.
...
Since A and B share outcomes in common, then the probability of either event occurring is equal to the probability of each event occurring minus the probability that of A and B occurring. The subtraction of the intersection of the event events avoids double-counting.
