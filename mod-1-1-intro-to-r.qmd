...
R is the statistical software package that we will use throughout the course and RStudio is an interface that makes using R much easier.

You can download **R** at: <https://www.r-project.org/>

You can download **RStudio** at: <https://posit.co/download/rstudio-desktop/>

You can also use **Posit Cloud** from Posit (the company that makes R Studio and other products) at: <https://posit.cloud/>
...
R Programming for Beginners: \url{https://youtu.be/fDRa82lxzaU}

R Programming Tutorial: \url{https://youtu.be/_V8eKsto3Ug}
...
If you need to install a package, you can always use the \textbf{install.packages} command. For example, you could use \textbf{install.packages("dplyr")} to install the \textbf{dplyr} package.

Once a package is installed, you typically will not need to re-install it. You may need to update your packages, however, and this can be easily done. You can either use the function \textbf{update.packages} or update packages through the packages menu in RStudio.

In the following code, we clear the workspace (essentially erasing working memory) and load three packages.

First, we clear memory with the command \textit{rm(list = ls())}.

Second, we load the packages into memory by using the \textit{library} statement. For example, \textit{library(dplyr)} loads the \textbf{dplyr} package so we can use its functions later in our code.

To learn more, see the following links.

dplyr: \url{https://dplyr.tidyverse.org/}

lubridate: \url{https://lubridate.tidyverse.org/}

tidyverse: \url{https://www.tidyverse.org/}

kable: \url{https://cran.r-project.org/web/packages/kableExtra}
...
We can use the \textbf{assignment operator} in R to assign values to a variable. The assignment operator can be thought of as moving a value, character, date, or other form into a named variable. The assignment operator is "\<-", that is, "x2 \<- 1056" means assign the value 1056 to the variable x2.

For example, in the code below, we assign the numerical value of 2 to the variable \textbf{x1} and the numerical value of 4.5 to the variable \textbf{x2}.

We can assign the value contained in one variable in another. For example, we can assign \textbf{x2} to \textbf{x3} and also specify that the integer value of \textbf{x2} is assigned to \textbf{x3}.
