...
So, in general, for a data frame or tibble, *data[row,column]*. For example, *cardata[1:2, 2:3]* returns rows 1 to 2 and columns 2 to 3.
...
If the row or column is left blank, all values for the column are the result. For example, *cardata[,1]* would return all rows for the first column of data. On the other hand, *cardata[1:2,]* would return the first two rows and all the columns.
